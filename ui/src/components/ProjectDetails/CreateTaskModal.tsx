        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <div className="dependency-list">
              {existingTasks.filter((t) => !t.isCompleted).length === 0 ? (
                <small style={{ color: 'var(--gray-text)' }}>No pending tasks available</small>
              ) : (
                existingTasks
                  ...
                  .map((task) => (
                    <div key={task.id} className="dependency-item">
                      <input
                        type="checkbox"
                        id={`dep-${task.id}`}
                        checked={selectedDeps.includes(task.title)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            const newDeps = [...selectedDeps, task.title]
                            setSelectedDeps(newDeps)
                            setFormData({ ...formData, dependencies: newDeps })
                          } else {
                            const newDeps = selectedDeps.filter(d => d !== task.title)
                            setSelectedDeps(newDeps)
                            setFormData({ ...formData, dependencies: newDeps })
                          }
                        }}
            ...
            <small>Select tasks that must be completed first</small>
          </div>
